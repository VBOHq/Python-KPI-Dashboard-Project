...
income_KPI =['Gross Profit Margin','Operating Profit Margin','Net Profit Margin', 'EBITDA Margin', 'EBIT Margin', 'Operating Income', 'EBITDA Net Income','Operating Expenses Ratio']

balance_KPI =['Return on Assets (ROA)', 'Return on Equity (ROE)', 'Working Capital Ratio', 'Quick Ratio (Acid-Test Ratio)','Accounts Receivable Turnover', 'Inventory Turnover', 'Days Sales Outstanding (DSO)', 'Days Inventory Outstanding (DIO)']
...
app.layout = html.Div(style={'backgroundColor': data_science_colors['background'], 'color': data_science_colors['text']}, children=[
    html.H1("FROZEN FOOD FINANCIAL ANALYSIS DASHBOARD", style={'text-align':'center', 'background-color':'light gray','color':'#0c74da', "font-weight": "bold", "font-size": 22}),
    html.Br(),
    ...
    dcc.Dropdown(id='line-item-dropdown'),
    html.Br(),
        ...
        dbc.Col(
            dcc.Loading(id="kpi-loading", type="default", children=[
                dash_table.DataTable(
                    ...
                    style_data_conditional=[{'if': {'row_index': 0}, 'backgroundColor': 'skyblue'}]
                )
            ]),
